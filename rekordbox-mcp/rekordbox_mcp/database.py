...
class RekordboxDatabase:
    ...
    async def remove_track_from_playlist_by_content(self, playlist_id: str, track_id: str) -> bool:
        if not self.db:
            raise RuntimeError("Database not connected")
        ...
        def _inner():
            ...
            for s in songs:
                if str(s.ContentID) == str(track_id):
                    self.db.remove_from_playlist(int(playlist_id), s)
                    self.db.commit()
                    self._invalidate_content_cache()
                    return True
    ...
    async def update_track_metadata(self, track_id: str, metadata: Dict[str, Any]) -> bool:
        """Update basic track metadata including ReleaseYear.

        Blank values are ignored so the GUI can safely pass editable fields.
        Genre remains handled separately by update_track_genre() unless a caller
        explicitly provides "genre" here.
        """
        if not self.db:
            raise RuntimeError("Database not connected")
        ...
        def _inner():
            ...
            if year_raw not in [None, ""]:
                try:
                    ...
                    logger.warning(f"Ignoring invalid release year for track {track_id}: {year_raw!r}")

            if artist:
                artist_row = self._resolve_or_create("get_artist", "add_artist", artist)
                if artist_row is not None:
            ...
            if album:
                album_row = None
                try:
                    existing = self.db.get_album(Name=album)
                    album_row = existing.first() if hasattr(existing, "first") else next(iter(existing), None)
                except Exception:
                    album_row = None
                if album_row is None:
                    try:
                        album_row = self.db.add_album(album)
                    except TypeError:
                        album_row = self.db.add_album(album, artist=None)
                if album_row is not None:
                    content.AlbumID = album_row.ID

            if metadata.get("genre"):
                genre_row = self._resolve_or_create("get_genre", "add_genre", str(metadata["genre"]))
                if genre_row is not None:
                    content.GenreID = genre_row.ID

            self.db.commit()
            self._invalidate_content_cache()
            logger.info(f"Updated metadata for track {track_id}: {metadata}")
            return True
        ...
        try:
            ...
            if self.db and hasattr(self.db, "rollback"):
                self.db.rollback()
            raise RuntimeError(f"Failed to update track metadata: {str(e)}")


    async def update_track_genre(self, track_id: str, genre_name: str) -> bool:
        """
        Set the rekordbox Genre field for one active track.

        Rekordbox stores Genre as a relation, not a free text list. For the
        SORTED assistant, pass a semicolon-separated value such as:
            "House; Tech House; Warm Up"

        That whole string is created as/linked to a single Rekordbox Genre row,
        so smart playlists can use a "Genre contains Tech House" style rule.
        """
        if not self.db:
            raise RuntimeError("Database not connected")
        ...
        def _inner():
            ...
            if not content or getattr(content, "rb_local_deleted", 0) != 0:
                raise RuntimeError(f"Active track not found: {track_id}")

            clean = (genre_name or "").strip()
            if not clean:
                raise RuntimeError("Genre name cannot be blank.")

            genre_row = self._resolve_or_create("get_genre", "add_genre", clean)
            if genre_row is None:
                raise RuntimeError(f"Could not resolve or create genre: {clean}")

            content.GenreID = genre_row.ID
            self.db.commit()
            self._invalidate_content_cache()
            logger.info(f"Updated genre for track {track_id}: {clean}")
            return True

        try:
            return await asyncio.to_thread(_inner)
        except Exception as e:
            logger.error(f"Failed to update genre for track {track_id}: {e}")
            if self.db and hasattr(self.db, "rollback"):
                self.db.rollback()
            raise RuntimeError(f"Failed to update track genre: {str(e)}")
